# TODO:
# Fix jury number (Game Changers)
# Remove tie columns from votes_trunc function
...
def votes_correct_against(votes):
    ...
    tally = pd.DataFrame(index=list(votes.index), columns=['votes_correct', 'votes_against'])
    ...
    for i in range(0, l):
        ...
        try:
            ...
            correct_vote = []
        tally.loc[list(correct_vote), ['votes_correct']] = tally['votes_correct'] + 1

        # Votes against
        va = current.value_counts()
        vadf = pd.DataFrame(va)
        vadf.columns = ['votes_against']
        tally['votes_against'] = tally.votes_against.add(vadf.votes_against, fill_value=0)
...
def process_season(votes, time_line_prct=[0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0]):
    ...
    for prct in time_line_prct:
        ...
        votes_trunc = truncate_votes_thru_episode(votes, thru_episode)
        scores = scores_from_votes(votes_trunc)
        season_stats['scores'][prct] = scores
    return season_stats


def process_all_seasons(seasons, time_line_prct=[0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0], save_to_disk=False):
